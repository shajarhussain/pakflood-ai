...
export default function FloodApp() {
  ...
  // Fetch model status once
  useEffect(() => {
    fetchModelStatus().then(setModelStatus);
  }, []);

  // Fetch zones when user first enables the heatmap
  ...
  const handleGeolocate = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLoading(false);
        setError("Location access denied — click on the map to select a point");
      }
    );
  }, [runPrediction]);
    ...
    <div className="relative w-screen h-screen bg-slate-950 overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-[1000] flex items-center justify-between px-4 py-3 bg-slate-950/80 backdrop-blur-sm border-b border-white/10">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          {modelStatus && (
            <span
              className={`ml-1 px-2 py-0.5 rounded text-[10px] font-medium border shrink-0 ${
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            title={zonesAge ? `${zonesCount} points · ${zonesAge}` : "Load nationwide risk heatmap"}
          >
            {zonesLoading ? (
            ...
            {zonesLoading ? "Loading…" : showZones ? "Hide Heatmap" : "Risk Heatmap"}
            {showZones && zonesAge && (
        ...
        <div className="absolute top-14 left-1/2 -translate-x-1/2 z-[1000] max-w-md w-full px-4">
          <div className="px-4 py-2.5 rounded-xl bg-amber-900/40 border border-amber-500/30 backdrop-blur-sm text-center">
            <p className="text-amber-300 text-xs">
              <strong>Model not loaded</strong> — predictions will return placeholder 50% until the
              XGBoost artifact is found. Check backend logs for the load error.
            </p>
          </div>
        </div>
      )}

      {/* Zone heatmap legend — shown when heatmap is visible */}
        ...
        <div className="absolute bottom-8 left-4 z-[1000]">
          <div className="px-3 py-2.5 rounded-xl bg-slate-900/90 border border-white/10 backdrop-blur-sm">
            ...
            {zones?.metadata?.is_fresh === false && (
              <p className="text-amber-500/70 text-[10px] mt-2 border-t border-white/5 pt-2">
      ...
      {prediction && selectedLocation && !loading && (
